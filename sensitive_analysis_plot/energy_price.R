# This script performs a sensitivity analysis on changing energy price in TCO calculation 
...
# Read the Excel file containing energy price data
data3 <- read_excel("sensitive_analysis_data/energy_price.xlsx")

# Define columns to remove based on the specified ranges
cols_to_remove <- c(seq(-100, -50, by = 10), seq(50, 100, by = 10))
...
# Function to calculate TCO differences for a given state and vehicle type
...
ggplot() +
  ...
  theme_minimal() +
  labs(x = "Gasoline Price Discount Rate",
       y = "Electricity Price Discount Rate",
       fill = "Difference") +
...
# Save the plot
ggsave("energy.png", width = 16, height = 9, dpi = 800)
