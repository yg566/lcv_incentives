# This script performs a sensitivity analysis on changing depreciation rate in TCO calculation 
...
# Read the Excel file containing final value data
data3 <- read_excel("sensitive_analysis_data/final_value.xlsx")

# Define columns to remove based on the specified ranges
cols_to_remove <- c(seq(0.21, 0.26, by = 0.01))
cols_to_remove <- sprintf("%.2f", cols_to_remove)
...
calc_differences <- function(state, vehicle_type, data) {
  ...
  names(difference_data) <- seq(0.06, by = 0.01, length.out = ncol(difference_data))
  difference_data$EV_Mileage <- seq(0.06, by = 0.01, length.out = nrow(difference_data))
...
out$vehicle_type <- factor(out$vehicle_type,
                           levels = c("Small LCV", "Medium LCV", "Large LCV"))
...
ggplot() +
  ...
  theme_minimal() +
  labs(x = "Gasoline Vehicle Depreciation Rate",
       y = "EV Depreciation Rate",
       fill = "Difference") +
...
# Save the plot
ggsave("final.png", width = 16, height = 9, dpi = 800)
