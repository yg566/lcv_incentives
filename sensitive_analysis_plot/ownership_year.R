# This script performs a sensitivity analysis on changing the period of ownership in TCO calculation 
...
# Read the Excel file containing ownership year data
data3 <- read_excel("sensitive_analysis_data/ownership_year.xlsx")

# Define columns to remove based on specified values
cols_to_remove <- c(seq(0, 4, by = 1))
cols_to_remove <- sprintf("%.0f", cols_to_remove)
...
calc_differences <- function(state, vehicle_type, data) {
  ...
  names(difference_data) <- seq(5, by = 1, length.out = ncol(difference_data))
  difference_data$EV_Mileage <- seq(5, by = 1, length.out = nrow(difference_data))
...
ggplot() +
  ...
  theme_minimal() +
  labs(x = "Gasoline Vehicle Period of Ownership",
       y = "EV Period of Ownership",
       fill = "Difference") +
...
# Save the plot
ggsave("year.png", width = 16, height = 9, dpi = 800)
