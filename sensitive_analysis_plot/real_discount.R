# This script performs a sensitivity analysis on changing the real discount rate in TCO calculation 
...
# Read the Excel file containing real discount rate data
data3 <- read_excel("sensitive_analysis_data/real_discount.xlsx")
...
calc_differences <- function(state, vehicle_type, data) {
  ...
  names(difference_data) <- seq(0, by = 0.01, length.out = ncol(difference_data))
  difference_data$EV_Mileage <- seq(0, by = 0.01, length.out = nrow(difference_data))
...
ggplot() +
  ...
  theme_minimal() +
  labs(x = "Gasoline Vehicle Real Discount Rate",
       y = "EV Real Discount Rate",
       fill = "Difference") +
  ...
  scale_x_continuous(limits = c(0, 0.2), labels = number_format(accuracy = 0.01)) +
  scale_y_continuous(limits = c(0, 0.2), labels = number_format(accuracy = 0.01))

# Save the plot
ggsave("discount.png", width = 16, height = 9, dpi = 800)
